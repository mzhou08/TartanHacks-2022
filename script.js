...
const ExtensionID = "cmdnlfcchjpgpkepdjjdanecaiejnolb";
var firstClick = true;
var roomNumber = "";
...
$("#conference-listen-btn").click(function () {
  ...
  VoxeetSDK.conference
    ...
    .then((conference) => {
      ...
      // 2. Join the conference
      return VoxeetSDK.conference
        .listen(conference, listenOptions)
        .then(() => {
          // Load the Output Audio devices
          VoxeetSDK.mediaDevice
            .enumerateAudioDevices("output")
...
const startVideo = () => {
  ...
  VoxeetSDK.conference
    .startVideo(
      ...
      payloadConstraints.constraints.video
    )
    .then(() => {
      $("#start-video-btn").attr("disabled", true);
      $("#stop-video-btn").attr("disabled", false);
...
$("#start-audio-btn").click(() => {
  ...
  VoxeetSDK.conference
    ...
    .then(() =>
      VoxeetSDK.mediaDevice.selectAudioInput($("#input-audio-devices").val())
    )
    .then(() => {
      $("#start-audio-btn").attr("disabled", true);
      $("#stop-audio-btn").attr("disabled", false);
